...
def take_turn():
    	global front_min_value, right_min_value, left_min_value
    	global idx_left, idx_front, idx_right
	if left_min_value > 0.2:
		bot_movement.send_cmd(0.0, 0.5)
		time.sleep(8)
		bot.movement.send_cmd(0.0, 0.0)
	elif right_min_value > 0.2:
		bot_movement.send_cmd(0.0, -0.5)
		time.sleep(8)
		bot.movement.send_cmd(0.0, 0.0)
...
while not rospy.is_shutdown():
    while not IsBotNearObstacle and not rospy.is_shutdown():
        if front_min_value > 0.2 and right_min_value > 0.2 and left_min_value > 0.2:
            bot_movement.send_cmd(0.15, 0.0)
        elif left_min_value < 0.2 or right_min_value < 0.2 or front_min_value < 0.2:
